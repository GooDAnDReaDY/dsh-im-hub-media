...
//
// The same code serves both the China edition (Feishu, open.feishu.cn) and the
// international edition (Lark, open.larksuite.com); only the base URL differs.
//
...
/**
 * @param options - { baseUrl, appId, appSecret, mode, webhookPath, verificationToken, allowedUserIds, http, onMessage, logger }.
 * `baseUrl` defaults to Feishu (China); pass LARK_OPEN for the international
 * Lark edition.
 */
export function createFeishuAdapter(options) {
	const { baseUrl = FEISHU_OPEN, appId, appSecret, mode = 'websocket', webhookPath = '/feishu', verificationToken = '', allowedUserIds = [], http, onMessage, logger } = options;
	const OPEN = baseUrl;
