// dsh-im-hub-media — turn a DeepSeek Harness (dsh) agent into an IM assistant
// with Telegram media support (fork of dsh-im-hub).
//
// This is a dsh *bundle* plugin: package.json declares `dsh.bundle.patch` →
// ./cordis.patch.yml, which inserts the `im-hub-media` row into the profile's
...
// to the chat.
//
// Compared to upstream dsh-im-hub, the Telegram adapter additionally:
//   * receives voice/audio (STT: Deepgram primary, HuggingFace Whisper
//     fallback), photos without caption (vision via dsh-vision-bridge),
//     documents (Hermes-style allowlist, size cap, image/video documents
//     rerouted, small text files injected inline) and replies (the replied-to
//     message becomes context);
//   * sends outbound media: `MEDIA:/abs/path` markers in the agent reply are
//     turned into native sendPhoto/sendVoice/sendDocument calls and removed
//     from the text.
//
...
// -----------------------------------
// The plugin registers a `dsh-settings` namespace (`im-hub-media`) so the Web
// GUI's plugin-settings surface can edit the configuration visually instead of
...
export const name = 'dsh-im-hub-media';
 ...
 * `attachments` persists inbound images so vision-bridge can describe them.
 */
export const inject = ['agentDefaultModel', 'agents', 'sessions', 'loader', 'settings', 'attachments'];

/** Settings namespace this plugin owns (spelled here, mirrored in the browser half). */
export const SETTINGS_NAMESPACE = settingsNamespace('im-hub-media');
...
export const Config = z.object({
	...
	media: z.object({
		...
		stt: z.object({
			/** Deepgram API key (api.deepgram.com). Empty = skip Deepgram. */
			deepgramApiKey: z.string().role('secret').default(''),
			/** Deepgram model, e.g. nova-2. */
			deepgramModel: z.string().default('nova-2'),
			/** STT language hint (BCP-47, e.g. ru). */
			language: z.string().default('ru'),
			/** HuggingFace token for the Whisper fallback. Empty = skip fallback. */
			hfApiKey: z.string().role('secret').default(''),
			/** HuggingFace Whisper model id. */
			hfModel: z.string().default('openai/whisper-large-v3')
		}).default({}),
		/** Max inbound document bytes (public Bot API caps getFile at 20 MB). */
		maxDocBytes: z.number().default(20 * 1024 * 1024),
		/** Max text-file bytes injected inline into the agent message (.md/.txt). */
		maxTextInjectBytes: z.number().default(100 * 1024),
		/** Local cache dir for downloaded media. */
		cacheDir: z.string().default(join(homedir(), '.dsh/im-hub-media/cache'))
	}).default({})
});
...
export function unflatten(flat, entry) {
	return {
		...
		// Not exposed through the flat GUI card; carried through untouched.
		media: entry?.media
	};
}
...
export function apply(ctx, config) {
	...
	const sync = () => {
		...
		bridge.start().catch((error) => {
			ctx.logger?.warn?.(`dsh-im-hub-media: failed to start: ${error instanceof Error ? error.message : String(error)}`);
		});
	};

	// `settings` is an explicit injection above, so register on this plugin's
	// own lifecycle rather than through a deferred child context.
	const settings = ctx.settings.register(SETTINGS_NAMESPACE, flatSchema, {
		base: flatten(entry)
	});
	source = () => unflatten(settings.get() ?? {}, entry);
	ctx.effect(() => settings.watch(sync), 'dsh-im-hub-media settings watcher');
	ctx.on('dispose', () => {
