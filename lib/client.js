...
window.__ModuleLoader__.load({
	id: "@goodandready/dsh-im-hub-media",
	factory: (require) => {
		var module = { exports: {} };
		...
		// Ядро 0.1.2-rc.1 убрало dsh-client-runtime из таблицы модулей. Живой источник
		// snapshot-store — @deepseek-ai/dsh-client-store (staticModules ядра); старый
		// путь пробуем вторым; при отсутствии работаем без store (см. bind ниже).
		let runtime = null;
		for (const id of ["@deepseek-ai/dsh-client-store", "@deepseek-ai/dsh-client-runtime/client"]) {
			try { runtime = require(id); break; } catch (missing) { /* следующий */ }
		}
		...
		const tagId = "dsh-im-hub-media/settings-card.module.css";
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=" + JSON.stringify(tagId) + "]") === null) {
			const tag = document.createElement("style");
			tag.dataset.plugin = "dsh-im-hub-media";
		...
		var CardForm = class {
			...
			bind(project) {
				if (!runtime) {
					// Ядро без snapshot-store: статичный снимок без подписки; карточка
					// не перерисуется сама, но не уронит загрузчик.
		...
		function Field(props) {
			...
			const common = {
				id: `plugin-config-im-hub-media-${spec.field}`,
				label: t(labelKey),
