...
export class Bridge {
	...
	async start() {
		...
		for (const adapter of adapters) {
			try {
				...
				this.ctx.logger?.info?.(`dsh-im-hub: ${adapter.name} adapter started`);
			} catch (error) {
				this.ctx.logger?.warn?.(`dsh-im-hub: ${adapter.name} adapter failed to start: ${error instanceof Error ? error.message : String(error)}`);
			}
		}
	...
	async handleMessage(input) {
		const { platform, chatId, userId, text, attachments = [] } = input;
	...
	/**
	 * Build the user-message content blocks from an inbound IM input:
	 *   - instruction prefix, reply context and the message text go into a
	 *     leading text block;
	 *   - photos are committed to the attachments service and attached as
	 *     `image` blocks (dsh-vision-bridge describes them for text models);
	 *   - voice/audio are transcribed (Deepgram primary, HF Whisper fallback)
	 *     and the transcript becomes text;
	 *   - documents/videos are referenced by their cached path so the agent
	 *     can read them with its filesystem tools.
	 */
	async buildUserContent(input) {
		const { text, attachments = [], replyText } = input;
		const stt = this.config.media?.stt ?? {};
		const textParts = [];
		if (this.config.agent.instructionPrefix) textParts.push(this.config.agent.instructionPrefix);
		if (replyText && replyText.trim()) textParts.push(`[Ответ на сообщение: ${replyText.trim()}]`);
		const blocks = [];

		for (const att of attachments) {
			if (att.kind === 'photo') {
				try {
					const data = new Uint8Array(await readFile(att.path));
					...
					textParts.push(`[Не удалось приложить изображение: ${error instanceof Error ? error.message : String(error)}]`);
				}
				...
				try {
					...
					const transcript = await transcribeAudio({
						bytes,
						deepgramApiKey: stt.deepgramApiKey ?? '',
						deepgramModel: stt.deepgramModel ?? 'nova-2',
						language: stt.language ?? 'ru',
						hfApiKey: stt.hfApiKey ?? '',
						hfModel: stt.hfModel ?? 'openai/whisper-large-v3',
						logger: this.ctx.logger,
					});
					textParts.push(transcript
						? `[Голосовое сообщение, расшифровка: ${transcript}]`
						: '[Голосовое сообщение (не удалось расшифровать)]');
				} catch (error) {
					textParts.push('[Голосовое сообщение (не удалось расшифровать)]');
				}
			} else {
				textParts.push(`[Файл: ${att.path}${att.name ? ` (${att.name})` : ''}]`);
			}
		}

		if (typeof text === 'string' && text.trim()) textParts.push(text.trim());
		const textBlock = textParts.filter(Boolean).join('\n\n');
		if (textBlock) blocks.unshift({ type: 'text', text: textBlock });
		if (blocks.length === 0) blocks.push({ type: 'text', text: '(пустое сообщение)' });
		return blocks;
	}
	...
	async runTurn(chat, input) {
		...
		try {
			...
			chat.agent.followup(createUserMessage({
				content,
				source: { kind: 'plugin', plugin: 'dsh-im-hub-media', form: 'relay' }
			}));
