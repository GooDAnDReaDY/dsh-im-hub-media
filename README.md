...
Многоплатформенный IM-шлюз с **полной поддержкой медиа** для [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) (dsh). Общайтесь с вашим агентом из **Telegram** (голосовые сообщения, фото, файлы, документы), **Feishu (Lark)** и **Enterprise WeChat (WeCom)**.

### Возможности

- **Голосовые сообщения**: автоматическое распознавание речи входящих аудиозаметок через встроенный STT.
- **Медиафайлы**: бесшовная пересылка скриншотов, картинок и документов в сессию агента.
- **Безопасность**: белые списки пользователей и Chat ID.
...
为 DeepSeek Harness (dsh) 打造的**多平台即时通讯媒体网关**。支持通过 **Telegram**（语音条、照片、文件）、**飞书 (Lark)** 与 **企业微信 (WeCom)** 与智能体自由对话。

### 核心功能

- **语音消息自动转写**：收到语音便签自动调用 STT 转换为文本交由智能体处理。
- **富媒体双向交互**：原生支持图片、代码文件及各类文档附件的双向收发。
- **访问权限控制**：严格的 Chat ID 与用户白名单机制。
